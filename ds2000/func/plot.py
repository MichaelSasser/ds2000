...
def simple_plot(inst, title: str="", recorded: bool=False) -> None:
    ...
    df = ((inst.waveform.data(recorded) - p.y_ref - p.y_origin) * p.y_inc
          * 10.0 ** (-get_prefix(c_scale).divisor))  # y
    time = np.linspace(0.0, get_prefix(t_scale).value * 14, num=df.size)  # x

    ax.plot(time, df)
    plt.title(f'{inst.id.company.split(" ")[0]} {inst.id.model}', loc='left')
    plt.title(f'V={get_prefix(t_scale).value} {get_prefix(t_scale).prefix}s, '
              f'H={get_prefix(c_scale).value} {get_prefix(c_scale).prefix}V',
              loc='right')

    ax.set(xlabel=f'Time ({get_prefix(t_scale).prefix}s)',
    ...
    plt.ylim(-(get_prefix(inst.channel1.scale).value) * 8 / 2,
             get_prefix(inst.channel1.scale).value * 8 / 2)
    plt.show()
    print(
        f'lower: {(- inst.waveform.y_origin - inst.waveform.y_reference) * 10 ** get_prefix(inst.channel1.scale).divisor}')
    ...
    print(
        f'r.waveform.y_increment={inst.waveform.y_increment}')  # voltage value per unit
